...
export const fontsStyle = () => {
	...
	fs.readdir(app.path.build.fonts, function (err, fontsFiles) {
		if (fontsFiles) {
			// Проверка существования файла стилей для подключения
		if (!fs.existsSync(fontsFile)) {
			// Создаем файл, если его нет
			fs.writeFile(fontsFile, '', cb);
			let newFileOnly;
			for (let i = 0; i < fontsFiles.length; i++)  {
				// Запись подключения шрифтов в файл
				...
				// Если файл имеется, вывод сообщения
				console.log("Файл scss/base/_fonts.scss уже существует. Для обновления, его необходимо ликвидировать!");
			}
		}
	});
